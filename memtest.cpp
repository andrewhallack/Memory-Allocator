...
int main(int argc, char * argv[]) {
    ...
    while ((opt = getopt(argc, argv, "b:s:")) != -1) {
        ...
        case 'b':
            // sets block size
            // if not used, resorts to default block size
            block_size = my_atoi(optarg);
            break;
        case 's': 
            // sets total reserved memory
            // if not used, resorts to default mem size
            mem_size = my_atoi(optarg);
            break;
        default:
            // if incorrect key used, resorts to default
            // block size and mem size
            cout << "Error: invalid key. Using default memory allocation" << endl;
            block_size = 128;
            mem_size = 512 * 1024; 
            break;
        }
